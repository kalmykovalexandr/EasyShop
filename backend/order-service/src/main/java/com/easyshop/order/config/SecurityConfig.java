...
public class SecurityConfig {
    ...
    SecurityFilterChain c(HttpSecurity h) throws Exception {
        h.csrf(cs -> cs.disable()).sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).authorizeHttpRequests(r -> r.requestMatchers("/healthz", "/readyz", "/api/products/**").permitAll().requestMatchers("/api/admin/**").hasRole("ADMIN").anyRequest().authenticated()).addFilterBefore((req, res, chain) -> auth(req, res, chain), BasicAuthenticationFilter.class);
        return h.build();
    }
    ...
    void auth(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException {
        ...
        if (a != null && a.startsWith("Bearer ")) {
            try {
                Jws<Claims> j = Jwts.parserBuilder().setSigningKey(key()).build().parseClaimsJws(a.substring(7));
